...
accuracies = np.zeros(100, dtype = float)
for k in range(100):
    #model = None
    model = Sequential()
    model.add(InputLayer((sen_Len,),dtype='int32'))
...
#model.add(Bidirectional(LSTM(60, return_sequences=True)))
    model.add(Conv1D(128, 5, activation='relu'))
#model.add(CNN(60, return_sequences=True,name=i'lstm_layer'))
    model.add(GlobalMaxPool1D())
    model.add(Dropout(0.1))
#Must use relu first to avoid vanishing gradient (https://towardsdatascience.com/is-relu-after-sigmoid-bad-661fda45f7a2)
#relu prevents vanishing gradient (https://stats.stackexchange.com/questions/126238/what-are-the-advantages-of-relu-over-sigmoid-function-in-deep-neural-networks)
...
#for i in range(10):
    #indices = np.arange(combined_Val.shape[0])
    #np.random.shuffle(indices)
    #combined_Val = combined_Val[indices]
    #target_Val = target_Val[indices]
    #combined_Val = shuffle(combined_Val, random_state=0)
    #predictions = None
    predictions = model.predict(combined_Val)
    #predictions_bool = None
    predictions_bool = np.argmax(predictions, axis=1)
    print(target_Val.shape)
#target_Val = to_categorical(target_Val)
    print(target_Val.shape)
    #target_Val_Argmax = None
    target_Val_Argmax = np.argmax(target_Val, axis=1)
    print(predictions_bool.shape)
    print(classification_report(target_Val_Argmax, predictions_bool,digits=3))
    #predictions_prob = None
    predictions_prob = model.predict_proba(combined_Val)
    #target_cat = None
    target_cat = to_categorical(target_Val_Argmax)
    #auc = None
...
#matrix = metrics.confusion_matrix(target_Val.argmax(axis=1), y_pred.argmax(axis=1))
    #matrix = None
    matrix = metrics.confusion_matrix(target_Val_Argmax, y_pred.argmax(axis=1))
    #micro = None
