...
#Must use relu first to avoid vanishing gradient (https://towardsdatascience.com/is-relu-after-sigmoid-bad-661fda45f7a2)#relu prevents vanishing gradient (https://stats.stackexchange.com/questions/126238/what-are-the-advantages-of-relu-    over-sigmoid-function-in-deep-neural-networks)
...
SVM = svm.SVC(C=1.0, kernel='rbf', degree=3, gamma='auto', verbose=2, max_iter=10000, probability=True)
...
#integer_mapping = {x: i for i,x in enumerate(target_Val_Resampled)}
#for i,item in enumerate(target_Val_Resampled):
#    target_Val_Resampled[i] = float(target_Val_Resampled[i])
#target_Val_Resampled = [integer_mapping[word] for word in target_Val_Resampled]
